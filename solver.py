...
class Solver(object):
    def __init__(self, args, data_loader):
        ...
        if (self.multi_gpu) and (torch.cuda.device_count() > 1):
            ...
            self.FDSMPI = nn.DataParallel(self.FDSMPI)
        self.FDSMPI.to(self.device)

        self.lr = args.lr
        self.criterion = nn.MSELoss()
        self.optimizer = optim.Adam(self.FDSMPI.parameters(), self.lr)


    def save_model(self, iter_):
        f = os.path.join(self.save_path, 'FDSMPI_{}iter.ckpt'.format(iter_))
        torch.save(self.FDSMPI.state_dict(), f)
    ...
    def load_model(self, iter_):
        ...
        if self.multi_gpu:
            ...
            for k, v in torch.load(f):
                n = k[7:]
                state_d[n] = v
            self.FDSMPI.load_state_dict(state_d)
        else:
            self.FDSMPI.load_state_dict(torch.load(f))
    ...
    def test(self):
        del self.FDSMPI
        # load
        self.FDSMPI = FDS_MPI().to(self.device)
